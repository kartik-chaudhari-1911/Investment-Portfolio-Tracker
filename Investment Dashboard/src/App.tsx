...
// // import { AuthPages} from './components/SignIn/AuthPages';
// // import {NavbarSign} from './components/SignIn/NavbarSign';
...
import { Navbar } from './components/Navigation/Navbar';
import { Home } from './components/Home/Home';
import  AuthPages  from './components/SignIn/AuthPages'; // Import the sign-in component
import { PortfolioData, Asset } from './types/portfolio';
...
function App() {
  const [currentPage, setCurrentPage] = useState<'home' | 'portfolio' | 'news'>('home');
  const [selectedAsset, setSelectedAsset] = useState<Asset | null>(null);
  const [isSignIn, setIsSignIn] = useState(false); // State to manage sign-in page
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <Navbar 
        ...
        onSignInClick={handleSignInClick} // Pass the sign-in click handler
      />

      <main className="max-w-7xl mx-auto px-4 py-6">
        {isSignIn ? (
          <AuthPages /> // Render the sign-in component if isSignIn is true
        ) : (
          <>
            {currentPage === 'home' && (
              <Home data={mockPortfolioData} onAssetSelect={handleAssetSelect} />
